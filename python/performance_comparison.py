...
def save_results(tech,dataset,retrieved,matches,scores_all,encoding_time,matching_time,auc_pr,auc_roc,pcu,ep,accuracy,recallrate_at_K_range, sampleNpoints, ori_TM, rt_TM):    
    ...
    with open('results/'+tech+'_'+dataset+'-VPRBench'+ts_datetime+'.csv', 'a') as csvfile:
...
def draw_ROC_Curves(fpr_dict,tpr_dict,techniques,dataset):
    ...
    plt.title(dataset)
    plt.grid()     
    plt.savefig('results/'+dataset+'-ROCcurves'+'.png') 
#    plt.show()    
...
def draw_PR_Curves(prec_dict,recall_dict,techniques,dataset):   
    ...
    plt.title(dataset)    
    plt.grid()     
    plt.savefig('results/'+dataset+'-PRcurves'+'.png') 
#    plt.show()           
...
#        axs[0,itr].set_xticks(viewpoint_positions)
    ...
    fig.tight_layout()    
    fig.savefig('results/'+'VPR_FPS_vs_PlatformSpeed.png')
...
def draw_retrieval_vs_distance(distance_btw_matches_info, techniques, dataset):
    ...
    plt.title(dataset)    
    plt.grid()
    plt.savefig('results/'+dataset+'-Retrieval_vs_Distance_Curves'+'.png') 
#    plt.show() 
        
def compute_matches(retrieved_all, ground_truth_info, scores_all):
    matches=[]
    scores = []
    itr=0
    for retr in retrieved_all:

        ind = np.in1d(retrieved_all[retr], ground_truth_info[retr])
        if np.any(ind):
            matches.append(1)
            scores.append(np.asarray(scores_all[retr])[ind][0])
        else:
            matches.append(0)
            scores.append(scores_all[retr][0])
        itr=itr+1        
    return matches, scores

def performance_comparison(dataset_name, dataset_directory,vpr_techniques, ground_truth_info, query_all, retrieved_all, scores_all, encoding_time_all, matching_time_all, all_retrievedindices_scores_allqueries_dict,descriptor_shape_dict): #dataset_directory has a numpy file named ground_truth_new.npy
    ...
    for vpr_technique in vpr_techniques:          
        print(vpr_technique)
        matches, scores_all[vpr_technique] = compute_matches(retrieved_all[vpr_technique], ground_truth_info, scores_all[vpr_technique])
